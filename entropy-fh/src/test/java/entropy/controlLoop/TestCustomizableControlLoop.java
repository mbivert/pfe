...
public class TestCustomizableControlLoop {
    ...
    public void testWithNoConstraintedVJobs() {
        try {
            ...
            mockConfAdapter.useConfiguration(src);
            MockVirtualMachineBuilder vmBuilder = new MockVirtualMachineBuilder();
            DurationEvaluator eval = new MockDurationEvaluator(10, 5, 2, 2, 3, 6, 3, 1, 1);
            MockDriverFactory mockDrvFactory = new MockDriverFactory();
            DefaultVJobBuilderFactory vjobF = new DefaultVJobBuilderFactory();
            //vjobF.add(new PlasmaVJobBuilder(new DefaultVJobElementBuilder(new MockVirtualMachineBuilder()), null));

            VJobsPool queue = new FCFSPersistentQueue(vjobF, new File(RESOURCES_DIR + "2VJobs/"));
            CustomizableControlLoop loop = new CustomizableControlLoop(new Monitor(mockConfAdapter), queue, vjobF, eval, new TimedReconfigurationExecuter(mockDrvFactory));
            loop.setAssignTimeout(30);
            loop.setPlanTimeout(30);
            //loop.setMasterVJobFile(RESOURCES_DIR + "parts.txt");
            loop.runLoop();
