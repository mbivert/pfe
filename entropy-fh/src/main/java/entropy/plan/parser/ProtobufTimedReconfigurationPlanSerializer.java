...
public class ProtobufTimedReconfigurationPlanSerializer extends FileTimedReconfigurationPlanSerializer {
    ...
    public TimedReconfigurationPlan unSerialize(InputStream in) throws IOException, TimedReconfigurationPlanSerializerException {
        ...
        try {
            ...
            for (PBTimedReconfigurationPlan.TimedReconfigurationPlan.Instantiate m : p.getInstantiatesList()) {
                plan.add(convert(cfg, m));
            }

            for (PBTimedReconfigurationPlan.TimedReconfigurationPlan.VirtualMachineRename m : p.getVmRenamesList()) {
                plan.add(convert(cfg, m));
            }

            //Last to prevent application failures due to a node currently hosting a VM
    ...
    private static Migration convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Migration a) {
    ...
    private static Run convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Run a) {
        return new Run(cfg.getAllVirtualMachines().get(a.getIdVm()),
                cfg.getAllNodes().get(a.getIdN()), a.getStart(), a.getEnd());
    }

    private static Stop convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Stop a) {
        return new Stop(cfg.getAllVirtualMachines().get(a.getIdVm()),
                cfg.getAllNodes().get(a.getIdN()), a.getStart(), a.getEnd());
    }

    private static Startup convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Startup a) {
        return new Startup(cfg.getAllNodes().get(a.getIdN()), a.getStart(), a.getEnd());
    }

    private static Shutdown convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Shutdown a) {
        return new Shutdown(cfg.getAllNodes().get(a.getIdN()), a.getStart(), a.getEnd());
    }

    private static Suspend convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Suspend a) {
        return new Suspend(cfg.getAllVirtualMachines().get(a.getIdVm()),
                cfg.getAllNodes().get(a.getIdSrc()),
                cfg.getAllNodes().get(a.getIdDst()), a.getStart(), a.getEnd());
    }

    private static Resume convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Resume a) {
        return new Resume(cfg.getAllVirtualMachines().get(a.getIdVm()),
                cfg.getAllNodes().get(a.getIdSrc()),
                cfg.getAllNodes().get(a.getIdDst()), a.getStart(), a.getEnd());
    }

    private static Pause convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Pause a) {
        return new Pause(cfg.getAllVirtualMachines().get(a.getIdVm()), cfg.getAllNodes().get(a.getIdN()), a.getStart(), a.getEnd());
    }

    private static UnPause convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Unpause a) {
        return new UnPause(cfg.getAllVirtualMachines().get(a.getIdVm()), cfg.getAllNodes().get(a.getIdN()), a.getStart(), a.getEnd());
    }

    private static Instantiate convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.Instantiate a) {
        return new Instantiate(cfg.getAllVirtualMachines().get(a.getIdVm()), a.getStart(), a.getEnd());
    }

    private static VirtualMachineRename convert(Configuration cfg, PBTimedReconfigurationPlan.TimedReconfigurationPlan.VirtualMachineRename a) {
        return new VirtualMachineRename(cfg.getAllVirtualMachines().get(a.getIdOldName()), cfg.getAllNodes().get(a.getIdN()), a.getIdNewName(), a.getStart(), a.getEnd());
    }
    ...
    public void serialize(Instantiate a) {
        PBTimedReconfigurationPlan.TimedReconfigurationPlan.Instantiate.Builder b = PBTimedReconfigurationPlan.TimedReconfigurationPlan.Instantiate.newBuilder();
        b.setStart(a.getStartMoment()).setEnd(a.getFinishMoment())
                .setIdVm(a.getVirtualMachine().getName());
    ...
    public void serialize(Deploy a) {
        PBTimedReconfigurationPlan.TimedReconfigurationPlan.Deploy.Builder b = PBTimedReconfigurationPlan.TimedReconfigurationPlan.Deploy.newBuilder();
        b.setStart(a.getStartMoment()).setEnd(a.getFinishMoment())
                .setIdN(a.getNode().getName());
    ...
    @Override
    public void serialize(VirtualMachineRename a) {
        PBTimedReconfigurationPlan.TimedReconfigurationPlan.VirtualMachineRename.Builder b = PBTimedReconfigurationPlan.TimedReconfigurationPlan.VirtualMachineRename.newBuilder();
        b.setStart(a.getStartMoment()).setEnd(a.getFinishMoment())
                .setIdN(a.getHost().getName())
                .setIdOldName(a.getOldName()).setIdNewName(a.getNewName());
        planBuilder.addVmRenames(b.build());
    }
}
