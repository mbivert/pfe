...
//    }
    ...
    /**
     * equivalent of rule 2.2 in case of binCandidateLoad == maxBigItemSizes + sumSmallItemSizes:
     * if sumSmallItemSizes < binLoadInf then remove big candidates with size < binLoadInf-sumSmallItemSizes
     * and update binLoadInf as binRequiredLoad + the size of the smallest big remaining candidate
     * instantiate it if only one big candidate remains
     *
     * @param bin bin index
     * @return {@code true} if a filtering is performed
     * @throws choco.kernel.solver.ContradictionException
     *          on bins and loads variables
     */
    private boolean bigItemsKnapsackAdditionalFiltering(int bin) throws ContradictionException {
        boolean ret = false;
        int firstS = firstSmall[bin].get();
        int firstB = candidates[bin].nextSetBit(0);
        if (firstB == firstS) return false;
        int smallSizes = bTLoads[bin].get() - iSizes[firstB];
        int space = loads[bin].getInf() - smallSizes;
        if (space <= 0) return false;
        int i = candidates[bin].prevSetBit(firstS - 1);
        while (i > firstB && iSizes[i] < space) {
            candidates[bin].clear(i);
            bins[i].removeVal(bin, this, false);
            if (bins[i].isInstantiated()) {
                assignItem(i, bins[i].getVal());
            }
            i = candidates[bin].prevSetBit(i - 1);
        }
        if (i == firstB) {
            if (iSizes[firstB] < space) {
                fail();
            } else {
                assignItem(i, bin);
                ret = bins[i].instantiate(bin, this, false);
            }
        } else {
            ret = filterLoadInf(bin, bRLoads[bin].get() + iSizes[i]);
        }
        ret |= filterLoadSup(bin, bTLoads[bin].get());
        return ret;
    }
}