...
/**
 * A simple storable queue. All the vjobs are stored into a folder into a given format
 * ({@value #DEFAULT_EXTENSION} by default). VJob can be added (removed) either using the add(remove) method, or
 * by putting(deleting) the vjob in the folder. VJobs can be in any format supported by the
 * VJobBuilderFactory.
 *
...
public class FCFSPersistentQueue extends VJobsPool {
    ...
    public List<VJob> getRunningPriorities() {
        ...
        for (File f : files) {
            try {
                VJob l = this.builder.build(f.getPath());
                readed.add(l);
            } catch (IOException e) {
     ...
     * TODO: Fix where it already exists a vjob with the same id BUT with a different format (so extension)
     *
    ...
    public boolean add(VJob v) {
        try {
            DefaultVJobFileSerializerFactory.getInstance().write(v, new File(buildPath(v.id())).getPath());
        } catch (IOException e) {
    ...
    public VJob get(String id) {
        ...
        try {
            return this.builder.build(f.getPath());
        } catch (IOException e) {
